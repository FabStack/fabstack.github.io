...
import { Model_$reflection, view as view_1, update, init } from "./Index.js";
...
(function () {
    let program_5;
    const program_4 = Program_Internal_withReactSynchronousUsing((equal, view, state, dispatch_1) => lazyView2With(uncurry2(equal), uncurry2(view), state, dispatch_1), "elmish-app", ProgramModule_withConsoleTrace(ProgramModule_mkProgram(init, update, view_1)));
    try {
        let patternInput;
        try {
            let coders;
            ...
            const encoder_3 = Auto_generateBoxedEncoder_437914C6(Model_$reflection(), void 0, coders, void 0);
            const decoder_3 = Auto_generateBoxedDecoder_Z6670B51(Model_$reflection(), void 0, coders);
            const deflate = (x) => {
                try {
                    return encoder_3(x);
                }
    ...
    const mapUpdate = (userUpdate, msg_1, model_2) => {
        let patternInput_1;
        if (msg_1.tag === 1) {
            patternInput_1 = [model_2, Cmd_none()];
        }
        else {
